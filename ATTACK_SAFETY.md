...
6. **자격증명 인자 노출 금지(MUST)** — 토큰·비밀번호를 CLI 인자(`--token-a`·`--user-a-pw`·`--user-a-id` 등)로 전달하면 **평문으로 노출**된다.
   ...
   - **현재 유효한 완화책**: **단일 운영자 전용 호스트 + 운영과 분리된 전용 테스트 계정**으로만 실행하고 히스토리에 남지 않게 처리(예: 히스토리 미기록 세션). 아래 ①②는 **코드 지원 시** 적용 가능한 향후 방향이며 지금은 무효/미지원이다.
     - ① 환경변수 참조(`--token-a "$TOKEN"`)는 **프로세스 목록 노출을 막지 못한다** — 셸이 `$TOKEN`을 execve **전에 실제 값으로 확장**하므로 `/proc/<pid>/cmdline`엔 평문이 그대로 실린다. **셸 히스토리 노출만** 일부 완화된다.
     - ② stdin 파이프 전달은 프로세스 목록 노출을 실제로 없앨 수 있으나, `attack_*.py`가 **stdin 입력을 코드로 지원하지 않아 현재 실행 불가**(향후 별도 범위).
   - **명시 한계(현재 코드)**: 현재 `attack_*.py`는 **CLI 인자 입력만 지원**하며 env/stdin 경유 입력은 **코드에 구현돼 있지 않다**(env/stdin 지원은 향후 별도 범위). 그전까지는 **공유·멀티유저 호스트에서 실행하지 않고**, 단일 운영자 격리 환경의 전용 테스트 계정으로만 실행하는 것을 필수로 한다.
