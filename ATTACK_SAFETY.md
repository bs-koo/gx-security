...
6. **자격증명 인자 노출 금지(MUST)** — 토큰·비밀번호를 CLI 인자(`--token-a`·`--user-a-pw`·`--user-a-id` 등)로 전달하면 프로세스 목록(`ps`/작업관리자 `tasklist`)에 **평문으로 노출**된다. **공유·멀티유저 호스트에서는 실행하지 않는다.** 단일 운영자 환경의 전용 테스트 계정으로만 실행한다.
