...
> ④에서 **gx-audit이 자동 발사**하는 동적 검사는 표적을 스스로 유도할 수 있는 것들입니다 — 파라미터 기반 **SQLi·XSS**와 정적 후보를 받아 연계하는 **접근통제(IDOR/BFLA)**. 나머지 동적 6종(인증·세션·SSRF/리다이렉트·경로/업로드)은 전용 표적(`--probe`·`--redirect-target`·`--traversal-target` 등)이 필요해 **`gx-pentest`로 직접 발사**합니다. 즉 *동적 스킬 6종은 모두 존재·동작*하되, 통합 파이프라인이 표적까지 자동 유도하는 것은 그중 일부입니다.
