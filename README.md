...
| ① 실제 발사 | 취약점 클래스별 페이로드를 실제로 발사합니다. 현재 실제 발사되는 것은 **실재 4종** — SQLi(error·boolean·time-based + sqlmap) · XSS(7개 컨텍스트 마커 반사) · 접근통제(BFLA·IDOR 권한 교차 호출) · 인증·세션·JWT(JWT 변조·토큰 재사용·쿠키 속성)입니다. SSRF/오픈 리다이렉트, Path Traversal/파일 업로드는 **개발중(미제공)**입니다. | `attack_*.py` (+ 공용 `tools/dyn_session.py`) |
| ② 악용 확정 | 실재 4종을 클래스별 판정으로 악용을 확정합니다. SQLi는 DB 오류·지연(≥2.5s)·데이터 추출, XSS는 마커 반사(저장·DOM형은 Playwright로 브라우저 실행까지), 접근통제는 `2xx=취약`·`401/403=방어`, 인증·세션·JWT는 변조·재사용 토큰에 `2xx=취약`·`401/403=방어`로 판정합니다. | Playwright MCP · `dyn_session` |
...
> ④에서 **gx-audit이 자동 발사**하는 동적 검사는 현재 구현·동작하는 **실재 4종**입니다 — 파라미터 기반 **SQLi·XSS**, 정적 후보를 받아 연계하는 **접근통제(IDOR/BFLA)**, 그리고 로그인 계정과 보호 엔드포인트(`--probe`)가 모두 주어지면 전체 발사하는 **인증·세션·JWT**입니다. 인증 동적은 조합에 따라 3단계로 갈립니다 — 로그인 계정(`--user-a-id/pw`)+`--probe`면 전체 발사(`dynamic`), 로그인 계정만 있고 `--probe`가 없으면 쿠키 속성만 쏘는 `partial`, `--token-a`(로그인 생략)나 계정 미지정이면 발사 없이 `static-only` 정적 추정에 머뭅니다. 나머지 동적 2종(SSRF/오픈 리다이렉트 · Path Traversal/파일 업로드)은 **개발중이며 아직 제공되지 않습니다**. 즉 통합 파이프라인이 자동 발사하는 것은 실재 4종이고, 개발중 2종은 미제공입니다.
...
진단(정적)은 9종 전체를 다룹니다. 침투(동적)는 그중 **실재 4종** — SQL Injection · XSS · 접근통제(IDOR/BFLA) · 인증·세션·JWT — 을 실제 발사로 확정합니다. 나머지 2종(SSRF/오픈 리다이렉트 · Path Traversal/파일 업로드)은 **개발중(미제공)**이며, CSRF·민감정보 노출은 정적 진단 전용입니다.
...
- **스킬 14** — 통합 1(`auditing-web-application-security`) · 진단 9(`detecting-*`) · 침투 4(`exploiting-sql-injection` · `exploiting-xss-vulnerabilities` · `exploiting-broken-access-control` · `exploiting-auth-session`)
