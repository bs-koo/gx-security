...
```
/plugin marketplace add D:\SQ\security-plugin
# 이후 /plugin 메뉴에서 gx-security 설치
```
...
- ✅ 동적은 **로컬(`localhost`) 또는 전용 스테이징**에만. 운영 금지(코드로 차단됨).
- ✅ **테스트 데이터** 사용 — 저장형 XSS 마커 등이 DB에 남을 수 있음.
- ✅ 평소엔 **정적만**(완전 안전), 동적은 확인이 필요한 취약점에만 `--target` 지정.
- ⚠️ **사내 운영이 사설망(10.x/192.168.x)이면** scope_guard가 자동 허용하므로, 그 호스트를 차단 목록에 등록할 것.
- 공인 대상은 `SECURITY_PLUGIN_AUTHORIZED=1` + `--authorized` 동시 충족 시에만 허용(소유자 책임).
...
**Q. 운영에 실수로 쏠까 걱정된다.**
A. `scope_guard`가 `prod`/`www`/공인 대상을 코드로 차단한다. 단 사내 운영이 사설망이면 차단 목록 등록이 필요하다(6번 참고).
