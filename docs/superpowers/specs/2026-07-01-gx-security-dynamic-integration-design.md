...
- [x] `audit.py`가 동적 6종 발사(`DYNAMIC` dict + `run_access_dynamic` 경로 합산), `--params` 지정 시 6종 모두 실발사 확인
- [x] `tests/` 소스 복구 + 전체 통과(정적 스캐너 + 동적 익스플로잇터)
- [x] 문서(README/USAGE/전 SKILL.md)가 코드 실상과 100% 일치, 허위 0건
- [x] `scope_guard` 안전 게이트가 전 익스플로잇터에서 강제됨
- [x] `plugin.json`/`marketplace.json`/문서 버전 `0.3.0` 일치
- [x] CSRF 정적 전용·스캐너 3대 한계가 문서에 명시
